...
struct streamstate *getStreamState(ogg_sync_state *pstate, ogg_page *ppage,
	...
	if (bos) { // début de stream
		...
		/* Ecriture dans la structure du stream */
		pthread_mutex_lock(&mutex_streamstate);
		...
		pthread_mutex_unlock(&mutex_streamstate);
		/* Fin d'écriture */
	...
	if (respac == 1 && (! s->headersRead) &&
		...
		if (res != TH_ENOTFORMAT) {
			...
			if (type == TYPE_THEORA) {
				...
				if (pthread_create(&theora2sdlthread, NULL, draw2SDL, (void*)(long long int)s->serial) != 0) {
					// le cast a deux buts : se conformer au prototype qui requiert un void*, et être en fait un int
					fprintf(stderr, "Could not create Theora_to_SDL thread");
					exit(EXIT_FAILURE);
				}
